...
private:
  ...
  ros::Publisher odom_base_link_pub_;
...
LeicaTCA1100::LeicaTCA1100(std::string port, unsigned long baud)
{
  ...
  odom_prism_pub_ = n_.advertise<nav_msgs::Odometry>("odometry/leica_prism", 1);
  odom_base_link_pub_ = n_.advertise<nav_msgs::Odometry>("odometry/leica_base_link", 1);
  ros::Duration(3).sleep();
  setupSerial(port, baud);

}

LeicaTCA1100::~LeicaTCA1100()
{
 // serial_.write("%R1Q,112:In\r\n");
...
//R1P,0,0:1285,2.589233073929421,-4.685544496028653,-0.347101691508029,1090182,2.589034327772839,-4.685654317702944,-0.347083832446396,1096576
...
*/
    ...
    prism_odom_msg.pose.pose.position.x = transform_ENU_prism.getOrigin().getX();
    prism_odom_msg.pose.pose.position.y = transform_ENU_prism.getOrigin().getY();
    prism_odom_msg.pose.pose.position.z = transform_ENU_prism.getOrigin().getZ();
    boost::array<double, 36ul> cov_prism = {0.2,0,0,0,0,0,
				      0,0.2,0,0,0,0,
				      0,0,0.2,0,0,0,
				      0,0,0,0.2,0,0,
				      0,0,0,0,0.2,0,
    ...
    base_link_odom_msg.pose.pose.position.x = transform_ENU_base_link.getOrigin().getX();
    base_link_odom_msg.pose.pose.position.y = transform_ENU_base_link.getOrigin().getY();
    base_link_odom_msg.pose.pose.position.z = transform_ENU_base_link.getOrigin().getZ();
    boost::array<double, 36ul> cov_base_link = {0.2,0,0,0,0,0,
				      0,0.2,0,0,0,0,
				      0,0,0.2,0,0,0,
				      0,0,0,0.2,0,0,
				      0,0,0,0,0.2,0,
