...
thorvald_2d_nav::scan_detected_line actual_meas;
int total_points = 6, total_landmarks = 2; 
double d = 0.5; // distance between two wheels
double lambda_x, lambda_y, q, dt, bearing_norm, range_1;
bool landmarks_observed = false;
ros::Time current_time, last_time;
int end_row_info = 0, finale = 0;
...
MatrixXd Z = MatrixXd::Zero(2*total_landmarks,1); // Z
MatrixXd expectedZ = MatrixXd::Zero(2*total_landmarks,1); // expectedZ
MatrixXd K = MatrixXd::Zero((2*total_landmarks+3),2*total_landmarks); // Kalman gain
MatrixXd diff = MatrixXd::Zero(2*total_landmarks,1); // error
MatrixXd H = MatrixXd::Zero(2*total_landmarks,(2*total_landmarks+3)); // H - Jacobian
MatrixXd line_pho = MatrixXd::Zero(total_landmarks,1); // polar co-ordinates
MatrixXd line_theta = MatrixXd::Zero(total_landmarks,1); // polar co-ordinates
...
MatrixXd Q = MatrixXd::Identity((2*total_landmarks),(2*total_landmarks))* measurement_noise; // Measurement Noise
