...
line_pho(0,0) = sqrt(pow(landmarks_pose.x[0],2) + pow(landmarks_pose.y[0],2));
line_theta(0,0) = atan2(landmarks_pose.y[0],landmarks_pose.x[0]);
line_pho(1,0) = sqrt(pow(landmarks_pose.x[1],2) + pow(landmarks_pose.y[1],2));
line_theta(1,0) = atan2(landmarks_pose.y[1],landmarks_pose.x[1]);

line_pho(2,0) = sqrt(pow(landmarks_pose.x[2],2) + pow(landmarks_pose.y[2],2));
line_theta(2,0) = atan2(landmarks_pose.y[2],landmarks_pose.x[2]);
line_pho(3,0) = sqrt(pow(landmarks_pose.x[3],2) + pow(landmarks_pose.y[3],2));
line_theta(3,0) = atan2(landmarks_pose.y[3],landmarks_pose.x[3]);
}
